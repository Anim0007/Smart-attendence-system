...
function render_logs_table(mysqli $conn): void {
  ...
  // Date filter (attendance marked_at)
  $where[]  = "DATE(a.marked_at) BETWEEN ? AND ?";
  $params[] = $date_start;
  $params[] = $date_end;
  $types   .= "ss";
...
<head>
  ...
  <style>
    .device-card{ background:#fff; border-radius:18px; border:1px solid rgba(0,0,0,.08); box-shadow:0 14px 40px rgba(0,0,0,.12); overflow:hidden; }
    .device-table-wrap{ max-height:520px; overflow-y:auto; overflow-x:auto; }
    .device-table{ width:100%; min-width:1100px; margin:0; border-collapse:separate; border-spacing:0; }
    .device-table thead th{ position:sticky; top:0; z-index:3; background:#f1f5f9; color:#0f172a; font-size:12px; font-weight:800; text-transform:uppercase; letter-spacing:.06em; padding:14px 16px; text-align:left; border-bottom:2px solid #e5e7eb; }
    .device-table td{ padding:14px 16px; white-space:nowrap; color:#0f172a !important; font-weight:600; border-bottom:1px solid #e5e7eb; }
    .device-table tbody tr:nth-child(odd){ background:#ffffff; }
    .device-table tbody tr:nth-child(even){ background:#f8fafc; }
    .device-table tbody tr:hover{ background:#e6f4f6; }
    .device-table td:first-child{ font-weight:900; color:#0ea5a4 !important; }

    .badge{ display:inline-flex; align-items:center; justify-content:center; padding:6px 14px; border-radius:999px; font-size:12px; font-weight:900; letter-spacing:.03em; border:1px solid; min-width:92px; }
    ...
    .filter-grid label{ font-size:12px; font-weight:800; text-transform:uppercase; letter-spacing:.05em; color:#0f172a; margin-bottom:6px; display:block; }
    .filter-grid select, .filter-grid input{
      width:100%; height:44px; border-radius:12px; border:1px solid #e5e7eb; padding:10px 12px; outline:none;
    }
    .filter-actions{ display:flex; gap:10px; justify-content:flex-start; }
    .btn-main{ height:44px; border:none; border-radius:20px; padding:0 16px; background:#0ea5a4; color:#fff; font-weight:900; cursor:pointer; }
    .btn-ghost{ height:44px; border-radius:20px; padding:0 16px; background:#fff; border:1px solid #e5e7eb; font-weight:800; cursor:pointer; }
...
<section class="container py-lg-5">
  <h1 class="slideInDown animated">Course Attendance Logs</h1>
